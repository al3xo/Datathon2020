...
def main():
    ...
    qry = '''
    WITH stateStats AS(
        SELECT DaysFromFirstDate, State, SUM(active) state_active, SUM(recovered) state_recovered, SUM(deceased) state_deceased
        FROM final
        GROUP BY DaysFromFirstDate, State
    )

    SELECT * FROM final f, stateStats ss
    WHERE f.State = ss.State
    AND ss.DaysFromFirstDate = f.DaysFromFirstDate
    '''
